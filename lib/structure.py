...
class BiplaneStructure:
    ...
    def __init__(self, h_RB, b_SC, h_SC, b_SW1_biax, b_SW1_foam, x2_SW1,
        ...
        self.upper_shear_web_1 = ShearWeb(
            num = 1,
            parent_structure = self,
            base_biax = b_SW1_biax_u,
            base_foam = b_SW1_foam_u,
            x2 = x2_SW1_u)
        self.upper_shear_web_2 = ShearWeb(
            num = 2,
            parent_structure = self,
            base_biax = b_SW2_biax_u,
            base_foam = b_SW2_foam_u,
            x2 = x2_SW2_u)
        self.upper_shear_web_3 = ShearWeb(
            num = 3,
            parent_structure = self,
            base_biax = b_SW3_biax_u,
            base_foam = b_SW3_foam_u,
            x2 = x2_SW3_u)
        self.upper_TE_reinforcement = TE_Reinforcement(
            parent_structure = self,
            base = b_TE_reinf_u,
            height_uniax = h_TE_reinf_uniax_u,
            height_foam = h_TE_reinf_foam_u)
        ...
        self.upper_aft_panel_1 = AftPanel(
            ...
            parent_structure = self,
            base = np.nan,
            height = h_aft_panel_1_u)
        self.upper_aft_panel_2 = AftPanel(
            num = 2,
            parent_structure = self,
            base = np.nan,
            height = h_aft_panel_2_u)
        self.upper_internal_surface_1 = InternalSurface(
            num = 1,
            parent_structure = self,
            base = np.nan,
            height_triax = h_int_surf_1_triax_u,
            height_resin = h_int_surf_1_resin_u)
        self.upper_internal_surface_2 = InternalSurface(
            num = 2,
            parent_structure = self,
            base = np.nan,
            height_triax = h_int_surf_2_triax_u,
            height_resin = h_int_surf_2_resin_u)
        self.upper_internal_surface_3 = InternalSurface(
            num = 3,
            parent_structure = self,
            base = np.nan,
            height_triax = h_int_surf_3_triax_u,
            height_resin = h_int_surf_3_resin_u)
        self.upper_internal_surface_4 = InternalSurface(
            num = 4,
            parent_structure = self,
            base = np.nan,
            height_triax = h_int_surf_4_triax_u,
            height_resin = h_int_surf_4_resin_u)
        self.upper_external_surface = ExternalSurface(
            parent_structure = self,
            base = np.nan,
            height_triax = h_ext_surf_triax_u,
            height_gelcoat = h_ext_surf_gelcoat_u)
